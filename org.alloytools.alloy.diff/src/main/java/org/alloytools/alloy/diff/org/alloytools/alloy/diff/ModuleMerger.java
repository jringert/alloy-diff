    ...
    private Expr replaceSigRefs(Expr expr) {
        switch (expr.getClass().getSimpleName()) {
            ...
            case "ExprQt" :
                ...
                for (Decl d : eq.decls) {
                    decls.add(new Decl(d.isPrivate, d.disjoint, d.disjoint2, replaceSigRefs(d.names), replaceSigRefs(d.expr)));
                }
            ...
            default :
                ...
                throw new RuntimeException("Error in replaceSigRefs for: " + expr.getClass().getSimpleName() + "\nat " + expr.pos);
        }
    }
            ...
            for (Field f2 : fields2) {
                if (f1.label.equals(f2.label)) {
                    if (f1.decl().expr instanceof ExprUnary && f2.decl().expr instanceof ExprUnary) {
                        ...
                        Expr e1mult = getArrowForOp(((ExprUnary) f1.decl().expr).op).make(f1.pos, f1.closingBracket, mergedSig, e1);
                        Expr e2mult = getArrowForOp(((ExprUnary) f2.decl().expr).op).make(f2.pos, f2.closingBracket, mergedSig, e2);
